...
def reset_ball(game):
    game['ball']['x'], game['ball']['y'] = WIDTH / 2, HEIGHT / 2
...
class PongWorker(HttpServer):
    ...
    def __init__(self, host, port):
        ...
        print(f"Pong Worker ready at {host}:{port}")
    # --- PERBAIKAN SELESAI ---
    ...
    def handle_request(self, method, path, body):
        ...
        try:
            if method == 'POST' and path == '/new_game':
                ...
                if game_id in games: games[game_id]['paddles'][player_id]['y'] = y; response_body = {"status": "ok"}
                else: raise FileNotFoundError
            elif method == 'GET' and path.startswith('/state/'):
                game_id = path.split('/')[-1]
                if game_id in games: update_game_state(game_id); response_body = games[game_id]
                else: raise FileNotFoundError
            else: status_code, status_text = 404, "Not Found"; response_body = {"error": "Endpoint not found"}
        except FileNotFoundError: status_code, status_text = 404, "Not Found"; response_body = {"error": "Game not found"}
        except Exception as e: status_code, status_text = 500, "Internal Server Error"; response_body = {"error": str(e)}
